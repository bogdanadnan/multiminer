...
bash scripts/benchmark-commits.sh "$mode-$machine" . .. "$work_factor" \
    "$max_memory_gb" "$min_threads" "$max_threads" "$min_t_cost" "$max_t_cost" \
    "$samples" "$b_mode" '' '' '' '' $branches 1>../bench.log 2>&1
EOF
